...
def fetch_version_info() -> str:
    """Fetch version information from PyPI for albumentationsx package.

    This function retrieves JSON data from PyPI containing information about
    the latest available version of albumentationsx. It handles network errors
    ...
    """
    opener = get_opener()
    url = "https://pypi.org/pypi/albumentationsx/json"
    try:
...
def check_for_updates() -> None:
    """Check if a newer version of albumentationsx is available on PyPI.
    ...
    try:
        ...
        if latest_version:
            ...
            if compare_versions(latest_parts, current_parts):
                warn(
                    f"A new version of AlbumentationsX is available: {latest_version!r} (you have {current_version!r})."
                    " Upgrade using: pip install -U albumentationsx. "
